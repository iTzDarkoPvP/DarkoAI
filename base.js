...
function openSettings() {
    ...
    themeToggle.classList.toggle('active', currentTheme === 'light');
    learningToggle.classList.toggle('active', learningEnabled);
    slowTalkToggle.classList.toggle('active', slowTalkEnabled);
}

function closeSettings(event) {    if (!event || event.target.id === 'settingsModal') {
...
function toggleWebSearchSetting() {
    ...
    if (searchBtn) searchBtn.classList.toggle('active', webSearchEnabled);
}
...
function toggleWebSearch() {
    ...
    btn.classList.toggle('active', webSearchEnabled);
    btn.title = webSearchEnabled ? '🔍 Búsqueda web activada' : 'Activar búsqueda web';
}

function toggleLearningSetting() {
    learningEnabled = !learningEnabled;
    const btn = document.getElementById('learningToggle');
    btn.classList.toggle('active', learningEnabled);
...
function toggleSlowTalkSetting() {
    ...
    const btn = document.getElementById('slowTalkToggle');
    const slowBtn = document.getElementById('slowTalkBtn');
    btn.classList.toggle('active', slowTalkEnabled);
    if (slowBtn) slowBtn.classList.toggle('active', slowTalkEnabled);
    localStorage.setItem('slowTalkEnabled', slowTalkEnabled.toString());
}
...
function createNewChat() {    const chatId = 'chat_' + Date.now();
...
function loadChat(chatId) {
    currentChatId = chatId;
    currentImages = [];    const previewArea = document.getElementById('imagePreviewArea');
...
function handleImageUpload(event) {
    ...
    files.forEach(file => {        if (!file.type.startsWith('image/')) return;
...
function createMessageElement(text, role, images) {
    ...
    parts.forEach(part => {
        if (part.type === 'text') {
            ...
            downloadBtn.textContent = 'Descargar';            downloadBtn.onclick = function() {
...
async function sendMessage() {
        ...
        try {
            ...
            const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
                ...
                body: JSON.stringify({
                    ...
                    messages: history,                    max_tokens: 4000,
                    temperature: 0.8
                })
            });
    ...
    try {
        ...
        const data = await res.json();        if (res.ok && data.choices && data.choices[0] && data.choices[0].message) {
            typingMsg.remove();
            const assistantMessage = data.choices[0].message.content;
            chat.messages.push({ role: 'assistant', text: assistantMessage });
            wrapper.appendChild(createMessageElement(assistantMessage, 'assistant'));
