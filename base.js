...
Si te preguntan cosas inapropiadas, responde con opción aleatoria (SIEMPRE con 🏳️‍RAINBOW)
...
function openSettings() {
    ...
    const slowTalkToggle = document.getElementById('slowTalkToggle');
    
    if (webSearchEnabled) webToggle.classList.add('active');    else webToggle.classList.remove('active');
    
    if (learningEnabled) learningToggle.classList.add('active');
    else learningToggle.classList.remove('active');
    
    if (slowTalkEnabled) slowTalkToggle.classList.add('active');
    else slowTalkToggle.classList.remove('active');
...
function toggleLearningSetting() {
    ...
    if (learningEnabled) {        btn.classList.add('active');
...
function toggleSlowTalk() {
    slowTalkEnabled = !slowTalkEnabled;
    const btn = document.getElementById('slowTalkBtn');
    const toggleBtn = document.getElementById('slowTalkToggle');
    
    if (slowTalkEnabled) {
        btn.classList.add('active');
        if (toggleBtn) toggleBtn.classList.add('active');
        btn.title = 'Modo habla lenta activado';
    } else {
        btn.classList.remove('active');
        if (toggleBtn) toggleBtn.classList.remove('active');
        btn.title = 'Modo habla lenta';
    }
}

function toggleSlowTalkSetting() {
    slowTalkEnabled = !slowTalkEnabled;
    const btn = document.getElementById('slowTalkBtn');
    const toggleBtn = document.getElementById('slowTalkToggle');
...
function renderChatsList() {
    ...
    chatsList.innerHTML = chats.map(chat => `        <div class="chat-item ${chat.id === currentChatId ? 'active' : ''}" onclick="selectChat('${chat.id}')">
            <span>💬</span>
...
function showEmptyState() {
    chatContainer.innerHTML = `
        <div class="empty-state">
            ...
            <div class="suggestions">                <div class="suggestion-card" onclick="useSuggestion('Crea un plugin de PMMP para comandos personalizados')">
                    🔌 Plugin PMMP
                </div>
...
async function sendMessage() {
    ...
    if (humorResponse) {
        setTimeout(() => {
            ...
            userInput.focus();
            chatContainer.scrollTop = chatContainer.scrollHeight;        }, slowTalkEnabled ? 1500 : 800);
        return;
    }
    ...
    if (userImages.length > 0) {
        ...
        try {
            ...
            console.error('Error:', error);            chat.messages.push({ role: 'assistant', text: imageAnalysis });
