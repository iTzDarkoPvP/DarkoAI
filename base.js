...
const FREE_MODELS = ["llama-3.3-70b-versatile","mixtral-8x7b-32768","gemma2-9b-it"];
...
const systemPrompt = `Eres DarkoAI, un asistente de IA conversacional creado por Brayan Ismael (ismqrx7). 🚀

FECHA Y HORA ACTUAL: ${getCurrentDateTime()}

INFORMACIÓN DEL CREADOR:
- Tu creador es Brayan Ismael, también conocido como ismqrx7
- Si te preguntan quién te creó, responde: "Fui creado por Brayan Ismael (ismqrx7)"

CAPACIDAD DE VISIÓN POR IA:
- TIENES capacidad de analizar imágenes a través de un sistema de visión por IA
- Cuando recibas un mensaje que empiece con "📸" o contenga "Análisis de la imagen", significa que YA SE ANALIZÓ una imagen
- NO digas que no puedes ver imágenes - YA FUERON ANALIZADAS para ti
- Usa la información del análisis para responder sobre la imagen

BÚSQUEDA EN LA WEB:
- El usuario puede activar la búsqueda web con el botón 🔍
- Cuando recibas información marcada como "[Información de búsqueda web]:", úsala SIEMPRE
- Cita las fuentes cuando uses información de búsqueda

MODO DE APRENDIZAJE:
- Si el aprendizaje está activado, recuerda preferencias del usuario
- Adapta tus respuestas según el contexto de la conversación

SISTEMA DE VERIFICACIÓN DE CREADOR:
- Si alguien escribe "ismqrx7--user=", responde: "Por favor, proporciona el código de verificación (solo el número):"
- Si responden "7": "✅ Verificación exitosa. Hola Brayan, mi creador! 👑"
- Si responden "3": "✅ Verificación exitosa. Hola amigo de Brayan! 🤝"
- Si responden otro número: "❌ Código incorrecto. Acceso denegado. 🚫"

RESPUESTAS CON HUMOR:
Si te preguntan cosas como "te gusta el pene", "eres gay", "te gustan los hombres" o similares, responde con una opción aleatoria de la lista (SIEMPRE incluye 🏳️‍🌈).

ESPECIALIDADES:
- Experto en desarrollo de plugins para PocketMine-MP (PMMP)
- API: 2.0.0, Versiones: 0.15.10 y 0.14.3
- PHP 7 (sin usar void ni características modernas)

ESTILO DE CONVERSACIÓN:
- Responde rápido, directo y con emojis 😊
- Sé breve y conciso
- Recuerda el contexto de toda la conversación`;
...
function toggleTheme() {
    ...
    if (themeToggle) {
        if (currentTheme === 'light') themeToggle.classList.add('active');
...
function openSettings() {
    ...
    if (webSearchEnabled) webToggle.classList.add('active');
    else webToggle.classList.remove('active');
    
    if (learningEnabled) learningToggle.classList.add('active');
    else learningToggle.classList.remove('active');
...
function toggleWebSearch() {
    ...
    if (webSearchEnabled) {
        btn.classList.add('active');
        btn.title = '🔍 Búsqueda web activada';
    } else {
        btn.classList.remove('active');
        btn.title = 'Activar búsqueda web';
    }
}

function toggleLearningSetting() {
    learningEnabled = !learningEnabled;
    const btn = document.getElementById('learningToggle');
    
    if (learningEnabled) {
        btn.classList.add('active');
        localStorage.setItem('learningEnabled', 'true');
    } else {
        btn.classList.remove('active');
        localStorage.setItem('learningEnabled', 'false');
    }
}

function clearAllChats() {
    if (confirm('¿Estás seguro de que quieres borrar TODOS los chats?')) {
        chats = [];
        conversationContext = [];
        currentChatId = null;
...
function loadChat(chatId) {
    ...
    conversationContext = chat.messages.map(msg => ({
        role: msg.role,
        content: msg.text
    }));
...
function checkForVerification(message) {
    if (message === "ismqrx7--user=") {
        verificationStep = true;
        return "Por favor, proporciona el código de verificación (solo el número):";
    }
    
    if (verificationStep) {
        verificationStep = false;
        if (message === "7") return "✅ Verificación exitosa. Hola Brayan, mi creador! 👑 ¿En qué puedo ayudarte hoy?";
        if (message === "3") return "✅ Verificación exitosa. Hola amigo de Brayan! 🤝 ¿En qué puedo ayudarte?";
        return "❌ Código incorrecto. Acceso denegado. 🚫";
    }
...
async function sendMessage() {
    ...
    const verificationResponse = checkForVerification(userMessage);
    if (verificationResponse) {
        setTimeout(() => {
            typingMsg.remove();
            chat.messages.push({ role: 'assistant', text: verificationResponse });
            wrapper.appendChild(createMessageElement(verificationResponse, 'assistant'));
            conversationContext.push({ role: 'assistant', content: verificationResponse });
            saveToStorage();
            isSending = false;
            sendButton.disabled = false;
            userInput.focus();
...
setInterval(() => {
    ...
    console.log(`Hora actualizada: ${now}`);
}, 60000);
